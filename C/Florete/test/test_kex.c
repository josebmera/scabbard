...
int test_kem_cca()
{
  ...
  uint8_t pk[rlwr_PUBLICKEYBYTES];
  uint8_t sk[rlwr_SECRETKEYBYTES];
  uint8_t c[rlwr_BYTES_CCA_DEC];	
  uint8_t k_a[rlwr_KEYBYTES], k_b[rlwr_KEYBYTES];
  ...
  uint64_t CLOCK_kp,CLOCK_enc,CLOCK_dec;
	...
	printf("rlwr_INDCPA_PUBLICKEYBYTES=%d\n", rlwr_INDCPA_PUBLICKEYBYTES);
	printf("rlwr_INDCPA_SECRETKEYBYTES=%d\n", rlwr_INDCPA_SECRETKEYBYTES);
	printf("rlwr_PUBLICKEYBYTES=%d\n", rlwr_PUBLICKEYBYTES);
	printf("rlwr_SECRETKEYBYTES=%d\n", rlwr_SECRETKEYBYTES);
	printf("rlwr_KEYBYTES=%d\n", rlwr_KEYBYTES);
	printf("rlwr_HASHBYTES=%d\n", rlwr_HASHBYTES);
 	printf("rlwr_BYTES_CCA_DEC=%d\n", rlwr_BYTES_CCA_DEC);
	printf("\n");
  	...
  	for(i=0; i<repeat; i++)
  	{
		...
		/*
		printf("ciphertext=\n");
		for(j=0; j<rlwr_BYTES_CCA_DEC; j++)
		printf("%02x", c[j]);
		printf("\n");
		*/
	...
	//printf("Average times polynomial mult: \t %lu \n",CLOCK_poly_mul/(repeat));
