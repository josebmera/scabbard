...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  for(i=0;i<mlwr_INDCPA_PUBLICKEYBYTES;i++)
    sk[i+mlwr_INDCPA_SECRETKEYBYTES] = pk[i];			      // sk[SABER_INDCPA_SECRETKEYBYTES:SABER_INDCPA_SECRETKEYBYTES+SABER_INDCPA_SECRETKEYBYTES-1] <-- pk	
...
{
  int i, fail;
  unsigned char cmp[mlwr_BYTES_CCA_DEC];
  unsigned char buf[64];
  unsigned char kr[64];                             // Will contain key, coins
 ...
 fail = verify(c, cmp, mlwr_BYTES_CCA_DEC);
