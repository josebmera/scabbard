...
void batch_64coefficient_multiplications_new(__m256i* a, __m256i* b, __m256i* res_avx_output);
void toom_cook_4way_avx_n1(__m256i* a_avx,__m256i* b_avx, __m256i *c_avx);
void mult(__m256i* a_avx,__m256i* b_avx, __m256i *res_avx);
...
//---------------AVX data-----------------------------
	...
	__m256i r0_avx, r1_avx, r2_avx,temp,r3_avx,r4_avx;
	__m256i aw_avx[5*small_len_avx1],bw_avx[5*small_len_avx1],cw_avx[2*5*small_len_avx1];
...
// EVALUATION
	...
	for (i=0; i<small_len_avx1; i++){
		r0_avx=a_avx[i];
		r1_avx=a_avx[i + small_len_avx1];
		r2_avx=a_avx[i + 2*small_len_avx1];
		temp=_mm256_add_epi16(r0_avx, r2_avx);
		aw_avx[i]= r0_avx; //a(0)
		aw_avx[4*small_len_avx1+i]= r2_avx; //a(inf)
		aw_avx[small_len_avx1+i]= _mm256_add_epi16(temp, r1_avx); //a(1)
		aw_avx[2*small_len_avx1+i]= _mm256_sub_epi16(temp, r1_avx); //a(-1)
		temp= aw_avx[2*small_len_avx1+i];
		temp= _mm256_add_epi16(temp, r2_avx);
		temp= _mm256_slli_epi16(temp,1);
		aw_avx[3*small_len_avx1+i]= _mm256_sub_epi16(temp, r0_avx); //a(-2)
	}
	for (i=0; i<small_len_avx1; i++){
		r0_avx=b_avx[i];
		r1_avx=b_avx[i + small_len_avx1];
		r2_avx=b_avx[i + 2*small_len_avx1];
		temp=_mm256_add_epi16(r0_avx, r2_avx);
		bw_avx[i]= r0_avx; //a(0)
		bw_avx[4*small_len_avx1+i]= r2_avx; //a(inf)
		bw_avx[small_len_avx1+i]= _mm256_add_epi16(temp, r1_avx); //a(1)
		bw_avx[2*small_len_avx1+i]= _mm256_sub_epi16(temp, r1_avx); //a(-1)
		temp= bw_avx[2*small_len_avx1+i];
		temp= _mm256_add_epi16(temp, r2_avx);
		temp= _mm256_slli_epi16(temp,1);
		bw_avx[3*small_len_avx1+i]= _mm256_sub_epi16(temp, r0_avx); //a(-2)
	}
	
	//CLOCK2=cpucycles();
	//CLOCK_TC_EVAL=CLOCK_TC_EVAL+(CLOCK2-CLOCK1);
	for (i = 0; i < 10*small_len_avx1; i++)
		cw_avx[i]=int0_avx;
	for (i = 0; i < 2*AVX_N1; i++)
		res_avx_output[i]=int0_avx;

	toom_cook_4way_avx_n1(&aw_avx[0],&bw_avx[0],&cw_avx[0]);
	toom_cook_4way_avx_n1(&aw_avx[small_len_avx1],&bw_avx[small_len_avx1],&cw_avx[2*small_len_avx1]);
	toom_cook_4way_avx_n1(&aw_avx[2*small_len_avx1],&bw_avx[2*small_len_avx1],&cw_avx[4*small_len_avx1]);
	toom_cook_4way_avx_n1(&aw_avx[3*small_len_avx1],&bw_avx[3*small_len_avx1],&cw_avx[6*small_len_avx1]);
	toom_cook_4way_avx_n1(&aw_avx[4*small_len_avx1],&bw_avx[4*small_len_avx1],&cw_avx[8*small_len_avx1]);

	for (i = 0; i < 2*small_len_avx1; i++) {
		
		r0_avx = cw_avx[i];
		r4_avx = cw_avx[8*small_len_avx1+i];
		r1_avx = _mm256_sub_epi16(cw_avx[2*small_len_avx1+i], cw_avx[4*small_len_avx1+i]);
		r1_avx = _mm256_srli_epi16(r1_avx, 1);
		r2_avx = _mm256_sub_epi16(cw_avx[4*small_len_avx1+i], cw_avx[0*small_len_avx1+i]);
		r3_avx = _mm256_sub_epi16(cw_avx[6*small_len_avx1+i], cw_avx[2*small_len_avx1+i]);
		r3_avx = _mm256_mullo_epi16 (r3_avx,inv3_avx);
		r3_avx = _mm256_sub_epi16(r2_avx, r3_avx);
		r3_avx = _mm256_srli_epi16(r3_avx, 1);
		temp = _mm256_slli_epi16(cw_avx[8*small_len_avx1+i], 1);
		r3_avx = _mm256_add_epi16(r3_avx, temp);
		r2_avx = _mm256_add_epi16(r2_avx, r1_avx);
		r2_avx = _mm256_sub_epi16(r2_avx, r4_avx);
		r1_avx = _mm256_sub_epi16(r1_avx, r3_avx);
		
		
		if(i<small_len_avx1){
			res_avx_output[0*small_len_avx1+i]=r0_avx;
			res_avx_output[1*small_len_avx1+i]=r1_avx;
			res_avx_output[2*small_len_avx1+i]=r2_avx;
			res_avx_output[3*small_len_avx1+i]=r3_avx;
			res_avx_output[4*small_len_avx1+i]=r4_avx;
		}
		else{
			res_avx_output[0*small_len_avx1+i]=_mm256_add_epi16 (res_avx_output[0*small_len_avx1+i], r0_avx);
			res_avx_output[1*small_len_avx1+i]=_mm256_add_epi16 (res_avx_output[1*small_len_avx1+i], r1_avx);
			res_avx_output[2*small_len_avx1+i]=_mm256_add_epi16 (res_avx_output[2*small_len_avx1+i], r2_avx);
			res_avx_output[3*small_len_avx1+i]=_mm256_add_epi16 (res_avx_output[3*small_len_avx1+i], r3_avx);
			res_avx_output[4*small_len_avx1+i]= r4_avx;
		}
	}
	//16*3=48=768/16
	/*for(i=0; i<48; i++)
		res_avx[i] = _mm256_sub_epi16(res_avx_output[i], res_avx_output[i+48]);*/
	
	//16*3=48=768/16
	for(i=0; i<24; i++)
	{
		res_avx[i] = _mm256_sub_epi16(res_avx_output[i], res_avx_output[i+48]);
		res_avx[i] = _mm256_sub_epi16(res_avx[i], res_avx_output[i+72]);
	}
	for(i=24; i<48; i++)
		res_avx[i] = _mm256_add_epi16(res_avx_output[i], res_avx_output[i+24]);
...
//---------------AVX data-----------------------------
	...
	__m256i aw_avx[7*small_len_avx],bw_avx[7*small_len_avx];
...
// EVALUATION
	...
	batch_64coefficient_multiplications_new(aw_avx, bw_avx, c_avx);//New

}

void batch_64coefficient_multiplications_new(__m256i* a, __m256i* b, __m256i* res_avx_output)//all 7 Karatsuba evaluation and interpolation are done in AVX.
{
	__m256i a_bucket[SCM_SIZE*4],b_bucket[SCM_SIZE*4],c_bucket[SCM_SIZE*4*2]; //SCM_SIZE = 16; Holds evaluation (a & b) for 7 Karatsuba at a time
